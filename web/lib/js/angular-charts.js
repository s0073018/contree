   ...
   */
     ...
     */
      function init() {
        ...
        setHeightWidth();
        //console.log("APRES SET HEIGHT : height =" + height);
        setContainers();
        //console.log("APRES SET CONTAINERS : height =" + height);
        var chartFunc = getChartFunction(chartType);
        chartFunc();
        //console.log("APRES DRAW GRAPHE : height =" + height);
        drawLegend();
        //console.log("APRES DRAW LEGEND: height =" + height);
     ...
     */
      function setContainers() {
        ...
        var hTitle = getChildrenByClassname(childrens, 'ac-title')[0].clientHeight;
     ...
     */
      function barChart() {
         ...
         */
          ...
          var y = d3.scale.linear().domain([0,15000]).range([height,0]);
          ...
          y.domain([0, parseInt(scope.score) + 100]);
         ...
         */
          ...
          filterXAxis(xAxis, x);
          // ****** GILLES  var yAxis = d3.svg.axis().scale(y).orient('left').ticks(10).tickFormat(d3.format('s'));
         ...
         * @type {[type]}
         */
          var hCalcul = height + margin.top + margin.bottom;
          if (hCalcul < 0) {
             // console.log("hCalcul inf 0 !!!");
          ...
          var svg = d3.select(chartContainer[0]).append('svg').attr('width', width + margin.left + margin.right).attr('height', hCalcul).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
     ...
     */
      function drawLegend() {
        ...
        if (chartType == 'pie') {
         ...
         /*angular.forEach(points, function (value, key) {
            scope.legends.push({
              color: getColorPartie(key),
              title: value.x
            });
          });*/
            scope.legends.push({
                color: getColor(0),
                title: 'Maines gagnees'
            });
            scope.legends.push({
                color: getColor(2),
                title: 'Maines gagnees (adversaire chute)'
            });
            scope.legends.push({
                color: getColor(3),
                title: 'Reste à faire'
      ...
      function getColorPartie(i) {
          if (i == 0) return config.colors[5];
          else return config.colors[6];
      }

      function getColorMaine(d, i) {
          //var pts = points[i];
          var t = d.data.type;

          if (t == 0) return config.colors[3];
          if (t == 1) {
              scope.alt = !scope.alt;
              if (scope.alt == true) return config.colors[0];
              else return config.colors[1];
          }
          if (t == 2) {
              scope.alt = !scope.alt;
              if (scope.alt == true) return config.colors[2];
              else return config.colors[7];
          }
...
angular.module("left", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("left",
    ...
    "<div class='ac-legend' style='max-width:25%;' ng-show='{{acConfig.legend.display}}'>\n" +
...
angular.module("right", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("right",
    ...
    "   <td><div ng-attr-style='background:{{l.color}}; height:15px;width:15px;'></div></td>\n" +
    "   <td style='padding-left:5px;color:white; display: inline-block;' ng-bind='l.title'></td>\n" +
    " </tr>\n" +
    " </table>\n" +
    "</div>");
}]);
